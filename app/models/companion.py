...
class Companion(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    personality_vector: Mapped[dict | None] = mapped_column(JSON)
...
class CompanionStateSnapshot(Base):
    ...
    appearance_state: Mapped[dict | None] = mapped_column(JSON)
    environment_state: Mapped[dict | None] = mapped_column(JSON)
...
class AdventureSession(Base, UUIDPrimaryKeyMixin):
    ...
    rewards_payload: Mapped[dict | None] = mapped_column(JSON)
